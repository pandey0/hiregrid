...
export default function GoogleConnectButton({ isConnected }: { isConnected: boolean }) {
  ...
  if (isConnected) {
    return (
      <div className="flex items-center gap-2 text-emerald-600 bg-emerald-50 px-3 py-2 rounded-xl border border-emerald-100">
        ...
        <span className="text-[13px] font-bold">Google Calendar Connected</span>
      </div>
    );
  }
  ...
  return (
    <Button
      ...
      disabled={isLoading}
      variant="outline"
      className="w-full h-11 border-slate-200 hover:bg-slate-50 rounded-xl font-bold transition-all gap-3"
    >

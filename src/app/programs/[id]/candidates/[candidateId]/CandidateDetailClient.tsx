          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <span className="font-mono text-[10px] font-black bg-blue-600 text-white px-2 py-0.5 rounded uppercase tracking-widest">
                  [ {candidate.status} ]
                </span>
              ...
              <h1 className="text-5xl font-black text-slate-900 tracking-tighter leading-none">{candidate.name}</h1>
              <p className="text-[16px] text-slate-500 mt-4 font-medium max-w-2xl leading-relaxed italic">
            ...
            <div className="flex items-center gap-4">
              ...
              {candidate.resumeUrl && (
                <Button asChild variant="outline" className="h-12 px-8 rounded-2xl border-slate-200 font-bold text-slate-600 hover:bg-slate-50 transition-all uppercase tracking-widest text-[11px]">
          ...
          <div className="lg:col-span-4 space-y-12">
            <section className="space-y-6">
              ...
              <div className="p-8 rounded-[40px] bg-white border border-slate-100 shadow-sm space-y-8">
                <div className="space-y-1">
                  <span className="font-mono text-[10px] font-bold text-slate-400 uppercase tracking-widest">Network Email</span>
                  <p className="text-[15px] font-bold text-slate-900">{candidate.email}</p>
                </div>
                {candidate.phone && (
                  <div className="space-y-1">
                    <span className="font-mono text-[10px] font-bold text-slate-400 uppercase tracking-widest">Phone Line</span>
                    <p className="text-[15px] font-bold text-slate-900">{candidate.phone}</p>
                  </div>
                )}
                {candidate.linkedIn && (
                  <div className="space-y-1">
                    <span className="font-mono text-[10px] font-bold text-slate-400 uppercase tracking-widest">Digital Identity</span>
                    <a href={candidate.linkedIn} target="_blank" rel="noopener noreferrer" className="block text-[15px] font-bold text-blue-600 hover:underline">LinkedIn Profile //</a>
                  </div>
                )}
                <div className="pt-6 border-t border-slate-50">
                  <span className="font-mono text-[10px] font-bold text-slate-400 uppercase tracking-widest">Origin Source</span>
                  <p className="text-[13px] font-black text-slate-900 mt-1 uppercase tracking-tighter">
            ...
            {candidate.atsScore !== null && (
              <section className="space-y-6">
                ...
                <div className="p-8 rounded-[40px] bg-slate-900 text-white relative overflow-hidden shadow-2xl shadow-slate-200">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/20 blur-[60px] -mr-16 -mt-16" />
                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-6">
                      <span className="font-mono text-[10px] font-bold text-blue-400 uppercase tracking-widest">Optimization Score</span>
            ...
            <section className="space-y-10">
              ...
              {candidate.bookings.length === 0 ? (
                <div className="p-24 rounded-[48px] border-2 border-dashed border-slate-100 text-center bg-slate-50/30">
                  ...
                  <p className="text-[16px] text-slate-400 font-medium">No sessions have been documented for this candidate yet.</p>
                </div>
              ) : (
                <div className="space-y-8">
                  {candidate.bookings.map((booking: any, idx: number) => (
                    ...
                    >
                      <Card className={cn(
                        "rounded-[40px] border border-slate-100 shadow-sm overflow-hidden",
                        booking.status === "COMPLETED" ? "bg-white" : "bg-slate-50/50"
                      )}>
                        <CardContent className="p-8">
                          <div className="flex items-start justify-between gap-10 mb-8 pb-8 border-b border-slate-50">
                            <div>
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <h3 className="text-xl font-bold text-slate-900 tracking-tight">{booking.round.name}</h3>
                                  <p className="text-[13px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                                    Interviewer // <span className="text-slate-600">{booking.programPanelist.externalName}</span>
                                  </p>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-6">
                              ...
                              <span className={cn(
                                ...
                                booking.status === "COMPLETED" 
                                  ? (booking.verdict === "PASS" ? "bg-emerald-50 text-emerald-700" : booking.verdict === "FAIL" ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700")
                                  : booking.status === "CANCELLED" ? "bg-rose-50 text-rose-600" : "bg-slate-100 text-slate-400"
                              )}>
                          ...
                          {booking.status === "COMPLETED" ? (
                            <div className="space-y-8">
                                  ...
                                  {Array.from({ length: 5 }).map((_, i) => (
                                    <div 
                                      ...
                                      className={cn(
                                        ...
                                        i < (booking.score || 0) ? "bg-blue-600" : "bg-slate-100"
                                      )} 
                                    />
                                  ))}
                                </div>
                                <span className="text-lg font-black text-slate-900 tabular-nums">SCORE // {booking.score}/05</span>
                              </div>
                              ...
                              {booking.aiRubric && (
                                <div className="pt-6">
                                  <span className="font-mono text-[10px] font-bold text-blue-600 uppercase tracking-[0.3em] block mb-4">Intelligence Logic //</span>
                                  <div className="flex flex-wrap gap-2">
                                    {(booking.aiRubric as Rubric | null)?.focusAreas?.map((area: string, i: number) => (
                                      <div key={i} className="px-3 py-1 rounded-lg bg-blue-50 border border-blue-100 text-[11px] font-bold text-blue-700 uppercase tracking-tight">
                                        {area}
                                      </div>
                                    ))}
                                  </div>
                            ...
                            <div className="flex items-center gap-4 p-6 rounded-2xl bg-blue-50/50 border border-blue-100">
                              <div className="w-2 h-2 rounded-full bg-blue-600 animate-pulse" />
                              <p className="text-blue-700 text-[14px] font-black uppercase tracking-widest">
